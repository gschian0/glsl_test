...
vec3 color(vec2 z){
    ...
    z = sin(z*20.0-u_time)*10.;
    return vec3(z.x*z.y);
}

void main(){
    vec2 st = (gl_FragCoord.xy/u_resolution.xy)-offset;
   // st += abs(sin(u_time))-0.5 +abs(cos(u_time))-0.5;
    vec3 circ = circle(st,.7);
    for (float i = 0.6; i > 0.1; i -= .1 ){
    circ += circle(st,i);
};
    // circ += circle(st,.5);
    // circ += circle(st,.4);
    ...
    // circ += circle(st, .1);
    // circ += circle(st, .05);
   vec3 collly = vec3(0,1,0);
    st.x *= u_resolution.x/u_resolution.y;
    gl_FragColor = vec4(color(st/zoom)*circ*collly,1.0);
}
